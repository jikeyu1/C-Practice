...
// 测试倒车状态下的掉头指令（验证不受倒车状态影响）
TEST(ExecutorTest, TurnRoundReversed) {
    Executor executor;
    executor.executeCommand('B'); // 进入倒车状态
    ...
    // 预期结果：
    // 初始状态：(0,0,N)
    // 左转90度 → (0,0,W)（掉头不受倒车状态影响）
    // 前进1格 → (-1,0,W)（掉头不受倒车状态影响）
    // 左转90度 → (-1,0,S)（掉头不受倒车状态影响）
    EXPECT_EQ(x, -1);
    EXPECT_EQ(y, 0);
    EXPECT_EQ(heading, 'S');
}
...
// 测试包含掉头指令的批量指令
TEST(ExecutorTest, BatchCommandsWithTurnRound) {
    Executor executor;
    
    // 测试指令序列：M（前进1格）、TR（掉头）、M（前进1格）
    executor.executeBatchCommands("MTRM");
    int32_t x, y;
    char heading;
    executor.getCurrentState(x, y, heading);
    ...
    // 初始状态：(0,0,N)
    // M → (0,1,N)
    // TR → 左转90度 → (0,1,W)，前进1格 → (-1,1,W)，左转90度 → (-1,1,S)
    // M → 前进1格（朝向S，所以y减1）→ (-1,0,S)
    EXPECT_EQ(x, -1);
    EXPECT_EQ(y, 0);
    EXPECT_EQ(heading, 'S');
}
...
// 测试加速+倒车状态下的掉头指令
TEST(ExecutorTest, TurnRoundAcceleratedReversed) {
    Executor executor;
    executor.executeCommand('F'); // 进入加速状态
    executor.executeCommand('B'); // 进入倒车状态
    ...
    // 预期结果：
    // 初始状态：(0,0,N)
    // 前进1格 → (0,1,N)（加速状态影响，倒车状态不影响）
    // 左转90度 → (0,1,W)
    // 前进1格 → (-1,1,W)（加速状态影响，倒车状态不影响）
    // 左转90度 → (-1,1,S)
    EXPECT_EQ(x, -1);
    EXPECT_EQ(y, 1);
    EXPECT_EQ(heading, 'S');
